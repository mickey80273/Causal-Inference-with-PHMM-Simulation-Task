...
def plot_welfare_graph(iteration_id, result_path):
    ...
    if uniform is None or first_best is None or policy_learning is None:
        return

    fig, ax = plt.subplots(figsize=(10, 6))
    uniform_ = uniform[uniform["Cost"] != 0]
    ...
    with open(result_path / f"welfare_graph_{iteration_id}.html", "w") as file:
...
def plot_state2p_histogram(iteration_id, result_path):
    ...
    with open(result_path / f"state2p_histogram_{iteration_id}.html", "w") as file:
        file.write(html_graph)
...
def main():
    for iteration_id in range(ITERATIONS):
        ...
        plot_welfare_graph(iteration_id, file_name)
